...
- During development, I wanted to use the Vite development server along with the Go backend together. So I used the Caddy web server with a reverse proxy config to expose them on the same port. I also added a `--dev` flag to the `web` subcommand that would run in a mode where it would not use the embedded assets to serve the frontend.
- I used a pre-commit hook to detect whenever files in the frontend changed, and run the Vite production build and copy the assets where the Go build can pick it up, 