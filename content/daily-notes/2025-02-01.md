...
- During development, I wanted to use the Vite development server along with the Go backend together. So I used the Caddy web server with a reverse proxy config to expose them on the same port.
- I used a pre-commit hook to run the Vite production build and copy the assets where the Go build can pick